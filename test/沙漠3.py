...
class AutoBot:
    ...
    def game_internal_operations(self):
        ...
        if not kazhu:
            ...
            try:
                ...
                self.keyboard_controller.release('1')
                time.sleep(3)
                self.move_mouse_win32(2900, 0, duration=1)
                self.keyboard_controller.press('s')
                time.sleep(2)
                self.keyboard_controller.release('s')
    ...
    def load_all_templates(self):
        ...
        self.load_level_states("shamo3", shamo3_states)
    ...
    def main_loop(self):
        ...
        while True:
            ...
            try:
                # 1. 点击关卡编号（使用状态识别）
                if not self.click_level_with_states("shamo3", max_retry=5):
                    print("无法找到关卡选择，跳过本轮")
                    continue
                time.sleep(1)
