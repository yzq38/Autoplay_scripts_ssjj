...
class AutoBot:
    ...
    def game_internal_operations(self):
        ...
        if not kazhu:
            ...
            try:
                ...
                time.sleep(40)
                # 操作4: 使用Windows API模拟视角转动
                print("开始使用Windows API模拟视角转动")
                # 向左平移视角
                print("向左平移视角")
                self.move_mouse_win32(-1000, 0, duration=0.5)
                print("左键射击")
                self.mouse_click_win32('left')
                time.sleep(0.5)
                ...
                # 向右平移视角
                print("向右平移视角")
                self.move_mouse_win32(1700, 0, duration=0.5)
                print("左键射击")
                self.mouse_click_win32('left')
                time.sleep(0.5)
                self.keyboard_controller.press('4')
                ...
                time.sleep(45)
                print("向左平移视角")
                self.move_mouse_win32(-800, 0, duration=0.5)
                print("左键射击")
                self.mouse_click_win32('left')
                time.sleep(0.5)
                self.keyboard_controller.press('4')
    ...
    def load_all_templates(self):
        ...
        self.load_level_states("maya1", maya1_states)
    ...
    def click_template(self, template_name, confidence=0.85, delay=0.5, max_retry=3):
        """查找并点击模板图像 - 增强版本"""
    ...
    def click_level_with_states(self, level_name, confidence=0.7, delay=0.5, max_retry=3):
        """点击关卡，考虑所有可能的状态"""
        for i in range(max_retry):
    ...
    def main_loop(self):
        ...
        while True:
            ...
            try:
                # 1. 点击关卡编号（使用状态识别）
                if not self.click_level_with_states("maya1", max_retry=5):
                    print("无法找到关卡选择，跳过本轮")
                    continue
                time.sleep(1)
                ...
                # 7. 点击仅好友左侧方块
                if not self.click_template("friend_only"):
                    print("无法找到仅好友选项")
                    continue
                time.sleep(1)
                ...
                # 11. 执行硬编码的游戏内操作（只有成功进入游戏才会执行到这里）
                print("确认成功进入游戏，开始执行游戏内操作...")
                kazhule = self.game_internal_operations()
                if kazhule:
                    continue


                # 12. 等待游戏结束，进入结算页面（专注模式）
                print("等待游戏结束，进入结算等待模式...")
                time.sleep(0.5)
