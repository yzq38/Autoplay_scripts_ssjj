...
class AutoBot:
    ...
    def game_internal_operations(self):
        ...
        try:
            ...
            self.mouse_down_win32('left')
            time.sleep(0.5)
            self.move_mouse_win32(-600, 0, duration=1)
            for i in range(270):
                if self.find_success(0.8):
                    time.sleep(1)
                    print("等待结算")
                ...
                if (i + 10) % 32 == 10:
                    ...
                    self.mouse_click_win32('middle')
                    self.mouse_down_win32('left')
                if self.find_boss_ready(0.8):
                    a = self.boss_ready_operations()
                    if a:
                        break
    ...
    def load_all_templates(self):
        ...
        self.load_level_states("diyv3", diyv3_states)
    ...
    def click_template(self, template_name, confidence=0.6, delay=0.5, max_retry=7):
        """查找并点击模板图像 - 增强版本"""
    ...
    def main_loop(self):
        ...
        while True:
            ...
            try:
                # 1. 点击关卡编号（使用状态识别）
                if not self.click_level_with_states("diyv3", max_retry=5):
                    print("无法找到关卡选择，跳过本轮")
                    continue
                time.sleep(1)
